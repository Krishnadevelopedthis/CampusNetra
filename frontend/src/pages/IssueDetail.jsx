...
export default function IssueDetail() {
  ...
  const createWO = useMutation({
    ...
    onError: (err) =>
      toast.error(
        err.detail || 'Could not create work order',
      ),
  })
  ...
  if (!issue) {
    return (
      <ErrorState
        error={{
          detail: 'We could not load this issue.',
        }}
        onRetry={refetch}
      />
    )
    ...
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <Link
            to="/issues"
            className="inline-flex items-center gap-1.5 text-body-md text-ink-muted hover:text-ink mb-2"
          >
            <ArrowLeft size={15} />
            Back to issues
          </Link>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {staff &&
            issue.allowed_transitions?.length >
              0 && (
              <Select
                value=""
            ...
            <dl className="space-y-3">
              ...
              <Row
                ...
                value={
                  issue.location
                    ?.room_name
                    ? `${issue.location.room_name} (${issue.location.room_code})`
                    : null
                }
              />
          ...
          {issue.ai && (
            <div className="ai-surface p-widget">
              ...
              {issue.ai.was_overridden && (
                <p className="text-body-sm text-warning-text mt-2">
                  Manually reclassified by
                  staff.
                </p>
              )}
            </div>
            ...
            <dl className="space-y-3">
              ...
              <Row
                ...
                value={
                  issue.work_order_reference
                    ? (
                      <span className="font-mono text-mono-data text-secondary">
                        {
                          issue.work_order_reference
                        }
                      </span>
      ...
      >
        <p className="text-body-md text-ink-muted mb-3">
          This is recorded on the issue
          timeline and the reporter is
          notified.
        </p>
...
function Row({ label, value }) {
  return (
    <div className="flex justify-between gap-3">
      <dt className="text-body-md text-ink-muted shrink-0">
        {label}
      </dt>
...
function DuplicatePanel({
  ...
  const merge = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        'Merged into the original issue.',
      )
      onDone()
    },
  ...
  const dismiss = useMutation({
    mutationFn: () =>
      api.post(
        `/issues/${issueId}/dismiss-duplicates`,
      ),
    ...
    <div className="widget border-warning-border bg-warning-bg p-widget">
      ...
      <div className="mt-3 space-y-2">
        {candidates.map((c) => (
          <div
            key={c.issue_id}
            className="flex flex-wrap items-center justify-between gap-3 p-3 rounded bg-surface border border-border-subtle"
          >
            <div className="min-w-0">
              ...
              <p className="text-body-md text-ink truncate mt-0.5">
                {c.title}
              </p>
